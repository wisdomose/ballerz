...
export default function StatPage() {
  ...
  return (
    <main className="max-w-7xl mx-auto pb-10">
      <nav className="flex justify-center gap-x-40 gap-y-5 sm:gap-20 sm:justify-between items-center flex-wrap py-3 border-b sm:px-6">
        ...
        <div className="flex items-center justify-evenly max-sm:w-full sm:gap-8">
      ...
      <div className="px-3 md:px-6">
        <div className="flex justify-between flex-wrap gap-2 border-b py-6 mb-6 md:mb-10 ">
          <h1 className="text-lg md:text-xl">
            {users[0].displayName}&apos;s Statistics
          </h1>
          <div className="flex justify-end items-center gap-6">
            <Button variant="outline" asChild>
              <Link href={`/add-stat?q=${params?.id ?? ""}`}>
                ...
                <ScrollArea className="mt-6 h-[85vh] pr-3">
                  {[...stats].map((entry) => {
                    return (
                      <div
                        key={entry.id}
                        className={`border rounded-md px-3 py-4 mt-3 ${
                          entry.id === stat.id ? "bg-primary text-white" : ""
                        }`}
                      >
                        <p
                          className={`text-xs ${
